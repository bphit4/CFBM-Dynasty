...
const get_initial_filter_option_groups = (subject, table_config, common) => {
  console.log({ table_config: table_config });
  let filter_option_groups = [];

  if (subject == "player stats" || subject == "world player stats") {
    filter_option_groups.push([
      {
        filter_key: "player_team_season.position_group",
        count: 0,
      ...
      {
        filter_key: "player_team_season.position",
        count: 0,
        display: "Position",
        icon: 'fa-football-ball',
        raw_options: [
    ...
    filter_option_groups.push([
      {
        filter_key: "player_team_season.class.class_name",
        count: 0,
        display: "Class",
    ...
    if (subject == "world player stats") {
      filter_option_groups.push([
        {
          filter_key: "player_team_season.team_season.conference_season.conference.conference_abbreviation",
          count: 0,
          display: "Conference",
          icon: 'fa-shield-alt',
        ...
        {
          filter_key: "player_team_season.team_season.team.school_name",
          count: 0,
          display: "Team",
          icon: 'fa-tshirt',
          raw_options: common.distinct(
  ...
  console.log({ filter_option_groups: filter_option_groups });
  ...
  //   }
  // }
  return filter_option_groups;
};

async function create_football_filters(common, table_config) {
  table_config.filters = table_config.filters || {};
  table_config.filters.filter_option_groups =
    table_config.filters.filter_option_groups ||
    get_initial_filter_option_groups(table_config.subject, table_config, common);
  ...
  var renderedHtml = await common.nunjucks_env.renderString(
    ...
    {
      filter_option_groups: table_config.filters.filter_option_groups,
...
const add_filter_listeners = async (common, table_config) => {
  ...
  $('.filter-option-dropdown-row').on('click', function(){
